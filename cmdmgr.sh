...
uninstall() {
    ...
    if [ "$response" = "Y" ]; then
        ...
        sed -i.bak '/Source shell command manager files/d' "$HOME/.zshrc"
        sed -i.bak '/shell-commands\/.*commands.sh/d' "$HOME/.zshrc"
...
case "$1" in
    ...
    "uninstall") uninstall ;;
    *) 
        echo "Usage: $(basename "$0") [create|list|install|uninstall]"
